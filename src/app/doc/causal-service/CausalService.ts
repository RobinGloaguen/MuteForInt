...
export class CausalService extends Service<causal.ICausalMsg, causal.ICausalMsg> {
  ...
  // Callbacks à réveiller dès qu'un état change (remplace le polling de waitUntil)
  ...
  // Notifie tous les waiters enregistrés qu'un état a changé
  ...
  // Attend sans polling que la condition soit vraie.
  // S'enregistre comme waiter et est réveillé dès qu'un état change.
  protected waitUntil(cond: () => boolean): Promise<void> {
